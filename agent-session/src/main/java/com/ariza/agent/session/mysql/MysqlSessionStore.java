package com.ariza.agent.session.mysql;
    ...
    public void post() {
        Boolean tableExists = jdbcTemplate.execute((ConnectionCallback<Boolean>) connection -> {
            ...
            try (ResultSet tables = connection.getMetaData()
                    .getTables(catalog, schema, null, new String[]{"TABLE"})) {
                return containsSessionMessagesTable(tables);
            }
        });
        if (!Boolean.TRUE.equals(tableExists)) {
            throw new IllegalStateException(
                    "启用 agents.session.type=mysql 需要预先创建数据表 " + TABLE_NAME);
        }
    }
